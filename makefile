...
FILES=transmitabm.cpp CProxyConfig.cpp CProxyManager.cpp CPairList.cpp CThreadProcess.cpp TThread.cpp CThreadProxyAbm.cpp DCExchDB.cpp DCSeriaOp.cpp DCABMClient.cpp


# set COMMON_INCS and COMMON_LIBS
COMMON_INCS:=-I$(PWD)/inc -I/usr/include -I$(DFM)/include  -I$(ITF)/include  -I$(TXML)/include  -I$(AVRO)/include -I$(DCC_INC_PATH) -I$(MQ)/include -I$(ZMQ)/include -I$(DCLOGCLI)/include -I$(ACE_PATH)/include

COMMON_LIBS:=-L$(DFM)/lib -L$(TXML)/lib -L$(DCC_LIB_PATH) -L$(MQ)/lib -L$(ZMQ)/lib -L$(SODIUM)/lib -L$(PGM)/lib -L$(DCLOGCLI)/lib -L$(AVRO)/lib
...
LinkLibs=  -ltinyxml -ldfm -ldcc -ldcmq -lzmq -lsodium -lpgm -lpthread -lrocketmq64 -ldclogcli -lavrocpp_s
